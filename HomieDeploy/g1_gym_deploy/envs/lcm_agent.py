...
class LCMAgent():
    ...
    def get_obs(self):
        ...
        观测向量组成（与 MuJoCo 版本一致）：
        1. 本体感受观测（48维）：
           - commands(3) * [2.0, 2.0, 0.5]
           - ang_vel(3) * 0.5
           - delta_yaw(1) * 0.5 = 0
           - delta_pose_x(1) * 0.5 = 0
           - delta_pose_y(1) * 0.5 = 0
           - gravity(3)
        ...
        base_pos = self.se.get_base_pos() #目前为0
              
        # 构建本体感受观测（48维）
        proprio_obs = np.zeros(48, dtype=np.float32)
        idx = 0
        
        # commands(3) * cmd_scale
        ...
        # (dof_pos[:12] - default_dof_pos[:12]) * 1.0
