...
class HumanoidBEAMDOJOCfg(LeggedRobotCfg):
    ...
    class env(LeggedRobotCfg.env):
        num_envs = 2048
        num_dofs = 12     # 机器人总自由度：上肢固定，只有12个腿部关节
        episode_length_s = 20.0 #与课程学习有关 
        
        n_scan = 225
        n_priv = 3
        n_priv_latent = 13 + 1 + 1 + 12 + 12 + 12 + 12 + 12  # mass(13) + fric(1) + rest(1) + motor(24) + offset(12) + Kp(12) + Kd(12)
        n_proprio = 45  # 上肢固定后：3(commands)+3(ang_vel)+3(gravity)+12(dof_pos)+12(dof_vel)+12(action_history)=45
        history_len = 10
