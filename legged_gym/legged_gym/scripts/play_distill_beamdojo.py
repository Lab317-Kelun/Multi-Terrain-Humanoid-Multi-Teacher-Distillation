...
def play(args):
    ...
    # 注意：MultiStudentTeacher.load_state_dict() 返回 bool（是否恢复训练），不是元组
    if "policy_state_dict" in state_dict:
        policy_state = state_dict["policy_state_dict"]
        print(f"Loading from 'policy_state_dict'")
        print(f"Policy state_dict keys: {list(policy_state.keys())}")
        result = policy.load_state_dict(policy_state, strict=False)
        print(f"Load result (resume_training flag): {result}")
    
    policy.eval()
    
    # 验证关键参数是否加载成功
    print("\n=== Model Loading Verification ===")
    print(f"Student network loaded: {hasattr(policy, 'student')}")
    print(f"Teacher network loaded: {hasattr(policy, 'teacher')}")
    print(f"Policy loaded_teacher flag: {policy.loaded_teacher}")
    print("=" * 40 + "\n")
    ...
    actions = torch.zeros(env.num_envs, 27, device=env.device, requires_grad=False)
