...
class G1_AMPLoader:
    ...
    def quaternion_slerp_torch(self, q0, q1, blend):
        if q0.dim() == 1:
            q0 = q0.unsqueeze(0)
        if q1.dim() == 1:
            q1 = q1.unsqueeze(0)
        if blend.dim() == 0:
            blend = blend.unsqueeze(0).unsqueeze(-1)
        ...
        out = s0 * q0 + s1 * q1
        out = torch.where(lin_mask, q0 + blend * (q1 - q0), out)
    ...
    def get_full_frame_at_time_batch(self, traj_idxs, times):
        ...
        rot_blend = self.quaternion_slerp_torch(all_frame_rot_starts, all_frame_rot_ends, blend)
    ...
    def blend_frame_pose(self, frame0, frame1, blend):
        ...
        blend_root_rot = self.quaternion_slerp_torch(
            root_rot0, root_rot1,
            torch.tensor(blend, dtype=torch.float32, device=self.device) if not torch.is_tensor(blend) else blend)
        if blend_root_rot.dim() == 1:
            blend_root_rot = -blend_root_rot if blend_root_rot[-1] < 0 else blend_root_rot
        else:
            mask = blend_root_rot[:, -1] < 0
            blend_root_rot = torch.where(mask.unsqueeze(-1), -blend_root_rot, blend_root_rot)
